...
angular.module('controllers',[]).controller('bloodPressureRecordCtrl',
    ['$scope','$timeout','$rootScope','$stateParams','$filter','$state','CreateDetection','GetControlTarget','Global',
        function ($scope,$timeout,$rootScope,$stateParams,$filter,$state,CreateDetection,GetControlTarget,Global) {
            ...
            $scope.submitForm = function(){
                if($scope.bloodPressure.diastolic){
                    ...
                    if(submitOnOff){
                        ...
                        CreateDetection.save({
                            ...
                            if(data.result == Global.FAILURE){
                                submitOnOff = true;
                                $scope.bloodTimeErrorHint = data.errorInfo;
                                $scope.bloodTimeError = true;
                                $timeout(function(){$scope.bloodTimeError = false;},1200);
                    ...
                    $scope.InfErrorHint = true;
                    $timeout(function(){$scope.InfErrorHint = false;},1200);
                }
            }
